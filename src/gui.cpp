...
TTF_Font* font;
u8 const* keys;
// Hack for external programs to edit the keyboard register
External external_keys = External();
...
void init() {
    ...
    window = SDL_CreateWindow("LaiNES", SDL_WINDOWPOS_CENTERED,
                              SDL_WINDOWPOS_CENTERED, WIDTH * last_window_size,
                              HEIGHT * last_window_size, 0);

    renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
    SDL_RenderSetLogicalSize(renderer, WIDTH, HEIGHT);

    gameTexture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888,
...
u8 get_joypad_state(int n) {
    ...
    if (useJoystick[n]) {
        ...
        j |= (SDL_JoystickGetButton(joystick[n], BTN_SELECT[n]))
             << 2;                                                    // Select.
    ...
    j |= external_keys.get_pressed_buttons();
...
void sendEvent(SDL_Event& sdlevent, SDL_Scancode scancode, uint32_t event) {
    sdlevent.type = event;
    sdlevent.key.keysym.scancode = scancode;

    if (!SDL_PushEvent(&sdlevent)) {
        std::cerr << "Failed to push keydown event" << std::endl;
    }

    external_keys.set_button(EXTERNAL_KEY_A, PressState::PRESSED);
...
/* Run the emulator */
void run() {
    // Hard Code the file to run for now...
	u8* cpu_ram_ref = CPU::get_ram_ref();

    const char* fileName = "Super Mario Bros (E).nes";
    ...
    while (true) {
        ...
        if (not pause) {
            CPU::run_frame();
            u8 dir = cpu_ram_ref[0x0005];
            std::cout << "direction: " << std::to_string(dir) << std::endl;
        }
        if (frameCounter % frameSkip == 0) {
            // std::cout << "Avg Frame Rate: " << 1000.0 / frameTime / frameSkip
        ...
        // if (frameCounter == 64 * 5) {
        //     std::cout << "Pressing start" << std::endl;
        //     SDL_Event pressStart;
        //     SDL_Event releaseStart;

        //     std::thread pressThread;
        //     std::thread releaseThread;

        //     pressThread = std::thread(sendEvent, std::ref(pressStart),
        //                               SDL_SCANCODE_RETURN, SDL_KEYDOWN);
        //     // releaseThread = std::thread(sendEvent, std::ref(releaseStart),
        //     //                             SDL_SCANCODE_RETURN, SDL_KEYUP);
