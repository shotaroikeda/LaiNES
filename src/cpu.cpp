...
template <bool wr> inline u8 access(u16 addr, u8 v = 0) {
    ...
    switch (addr) {
    case 0x0000 ... 0x1FFF:
        r = &cpu_ram[addr % 0x800];
        if (wr)
            *r = v;
        return *r; // cpu_RAM.
    case 0x2000 ... 0x3FFF:
        return PPU::access<wr>(addr % 8, v); // PPU.
    ...
    case 0x4017:
        if (wr)
            ...
            return Joypad::read_state(1); // Joypad 1.
    ...
    case 0x4016:
        if (wr) {
            ...
            return Joypad::read_state(0); // Joypad 0.
    case 0x4018 ... 0xFFFF:
        return Cartridge::access<wr>(addr, v); // Cartridge.
    }
    return 0;
}
...
void exec() {
    ...
    case 0xFE:
        return INC<_abx>();
    default: {
        std::cout << "Invalid OPcode! PC: " << PC << " OPcode: 0x" << std::hex
                  << (int)rd(PC - 1) << "\n";
        return NOP();
    }
    }
